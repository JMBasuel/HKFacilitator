...
import com.upang.hkfacilitator.databinding.ManagerFacuScheduleBinding
...
class ManagerFacuSchedule : Fragment(), ConnectionStateListener, SchedFaciClickListener {

    private lateinit var binding: ManagerFacuScheduleBinding
    ...
    override fun onCreateView(
        ...
        binding = ManagerFacuScheduleBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        dbRef.child("/Data/Facilitator").addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    binding.rvScheduleFaci.adapter = SchedFaciAdapter(facilitators, this@ManagerFacuSchedule,
    ...
    private fun approveExtension() {
        ...
        val notifyID = today.substring(2).replace("-", "") +
                "${todayHour().replace(":", "")}${schedule.id}APPROVEMA"
    ...
    private fun declineExtension() {
        val today = todayDate(true)
        val notifyID = today.substring(2).replace("-", "") +
                "${todayHour().replace(":", "")}${schedule.id}DECLINEMA"
